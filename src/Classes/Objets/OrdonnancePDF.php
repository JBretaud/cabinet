<?php
class OrdonnancePDF extends FPDF
{
// Page header
function Header()
{   
    ...
    if(isset($_GET['patient'])&&!empty($_GET['patient'])){
        $attributes=[];
        $table=explode(",",$_GET['patient']);
        foreach($table as $couple){
            $attribute=explode("=",$couple);
            $attribute[0]=str_replace(" ","",$attribute[0]);
    ...
    if(isset($_GET['praticien'])&&!empty($_GET['praticien'])){
        $attributes=[];
        $table=explode(",",$_GET['praticien']);
        ...
        $praticien=new Praticien($attributes);
    }else{
...
function Footer()
{
    ...
    $chemincode=explode("/",$praticien->getCheminPhoto());
        ...
        $chemincodefin.="Signature.jpg";
    
    
    
    $this->Image($chemincodefin,80,150,50);
...
public function pdfEdit(bool $dl){
    ...
    ($dl) ? $pdf->Output('D',$ajd->format('Y-m-d')."_Ordonnance_".$patient->getNom()) : $pdf->Output();
}
}
?>